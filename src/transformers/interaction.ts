...
export function transformInteraction(bot: Bot, payload: DiscordInteraction) {
  const guildId = payload.guild_id
    ...
    : undefined;
  const user = bot.transformers.user(
    bot,
  ...
  const interaction = {
    ...
    appPermissions: payload.app_permissions
      ? bot.transformers.snowflake(payload.app_permissions)
      : undefined,
    message: payload.message
      ? bot.transformers.message(bot, payload.message)
      : undefined,
    ...
    data: payload.data
      ? {
          ...
          id: payload.data.id
            ? bot.transformers.snowflake(payload.data.id)
            : undefined,
  ...
  if (guildId && resolved.members) {
    transformed.members = new Collection(
      Object.entries(resolved.members).map(([id, value]) => {
        const member: Member = bot.transformers.member(
          bot,
          value,
          guildId,
  ...
  if (resolved.attachments) {
    transformed.attachments = new Collection(
      Object.entries(resolved.attachments).map(([key, value]) => {
        ...
        return [
          id,
          bot.transformers.attachment(bot, value as DiscordAttachment),
        ];
      })
    );
  }
