import { DiscordApplicationCommand } from "@discordeno/bot";
...
export function transformApplicationCommandToDiscordApplicationCommand(
  ...
  return {
    ...
    guild_id: payload.guildId
      ? bot.transformers.reverse.snowflake(payload.guildId)
      : undefined,
    name: payload.name,
    ...
    options: payload.options?.map((option) =>
      bot.transformers.applicationCommandOption(bot, option)
    ),
