  ...
  const guild = {
      ...
      ? {
          ...
          welcomeChannels: payload.guild.welcome_screen.welcome_channels.map(
            (wc) => ({
              ...
              emojiId: wc.emoji_id
                ? bot.transformers.snowflake(wc.emoji_id)
                : undefined,
    ...
    members: new Collection(
      payload.guild.members?.map((member) => {
        const result = bot.transformers.member(
          bot,
          member,
          guildId,
    ...
    voiceStates: new Collection(
      (payload.guild.voice_states || [])
        .map((vs) =>
          bot.transformers.voiceState(bot, { voiceState: vs, guildId })
        )
        .map((vs) => [vs.userId, vs])
    ),
    ...
    permissions: payload.guild.permissions
      ? bot.transformers.snowflake(payload.guild.permissions)
      : 0n,
    afkChannelId: payload.guild.afk_channel_id
      ? bot.transformers.snowflake(payload.guild.afk_channel_id)
      : undefined,
    ...
    applicationId: payload.guild.application_id
      ? bot.transformers.snowflake(payload.guild.application_id)
      : undefined,
